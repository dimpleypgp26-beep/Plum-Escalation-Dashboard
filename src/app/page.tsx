...
export default function Dashboard() {
  ...
  const avgResponseTime = Math.round(
    data.filter((e) => e.responseTime).reduce((sum, e) => sum + (e.responseTime || 0), 0) /
      data.filter((e) => e.responseTime).length
  );
  const avgResolutionDays = Math.round(
    data.filter((e) => e.resolutionTime).reduce((sum, e) => sum + (e.resolutionTime || 0), 0) /
      data.filter((e) => e.resolutionTime).length / 24
  );
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
              ...
              <MetricCard
                ...
                value={slaBreached}
                color="bg-orange-50"
                subtitle={`${Math.round((slaBreached / (total - closed)) * 100)}% of active`}
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <div className="mt-4 grid grid-cols-2 gap-6">
                  <div>
                    <p className="text-3xl font-bold text-green-600">{Math.round((closed / total) * 100)}%</p>
                    <p className="text-sm text-gray-400">Resolution Rate</p>
                  </div>
                  <div>
                    <p className="text-3xl font-bold text-purple-600">{Math.round(((total - open) / total) * 100)}%</p>
