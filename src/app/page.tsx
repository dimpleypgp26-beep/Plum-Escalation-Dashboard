...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "var(--plum-bg)" }}>
      {/* Header */}
      <header style={{ background: "linear-gradient(135deg, #311B92 0%, #5E35B1 50%, #7C4DFF 100%)" }}>
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-semibold text-white">Escalation Dashboard</h1>
              <p className="text-[11px] text-white/60">AI-Powered Management System</p>
            </div>
          </div>
        ...
        {loading ? (
          <div className="flex h-64 items-center justify-center">
            <div className="text-center">
              <svg className="mx-auto h-8 w-8 animate-spin" style={{ color: "#5E35B1" }} fill="none" viewBox="0 0 24 24">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
                  <MetricCard
                    ...
                    color="bg-white"
                    accentColor="#5E35B1"
                    icon={<svg className="h-8 w-8" style={{ color: "#5E35B1" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>}
                  />
                  <MetricCard
                    label="Open"
                    value={open}
                    ...
                    color="bg-white"
                    accentColor="#7C4DFF"
                    icon={<svg className="h-8 w-8" style={{ color: "#7C4DFF" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                  />
                  <MetricCard
                    label="Critical"
                    value={critical}
                    ...
                    icon={<svg className="h-8 w-8" style={{ color: "#E91E63" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>}
                  />
                  <MetricCard
                    label="SLA Breached"
                    value={slaBreached}
                    ...
                    icon={<svg className="h-8 w-8" style={{ color: "#F57C00" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                  />
                  <MetricCard
                    label="Blocked"
                    value={blocked}
                    ...
                    icon={<svg className="h-8 w-8" style={{ color: "#5E35B1" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" /></svg>}
                  />
                </div>

                {/* TAT Summary */}
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div className="rounded-xl bg-white p-5 shadow-sm" style={{ border: "1px solid var(--plum-border)" }}>
                    <h3 className="text-sm font-semibold" style={{ color: "var(--plum-text)" }}>Turnaround Time</h3>
                    <div className="mt-4 grid grid-cols-2 gap-6">
                      <div>
                        <p className="text-3xl font-bold" style={{ color: "#5E35B1" }}>{avgResponseTime}h</p>
                        <p className="text-sm text-gray-400">Avg First Response</p>
                      </div>
                      <div>
                        <p className="text-3xl font-bold" style={{ color: "#5E35B1" }}>{avgResolutionDays}d</p>
                  ...
                  <div className="rounded-xl bg-white p-5 shadow-sm" style={{ border: "1px solid var(--plum-border)" }}>
                    <h3 className="text-sm font-semibold" style={{ color: "var(--plum-text)" }}>Quick Stats</h3>
                    <div className="mt-4 grid grid-cols-2 gap-6">
                      <div>
                      ...
                      <div>
                        <p className="text-3xl font-bold" style={{ color: "#7C4DFF" }}>{total > 0 ? Math.round(((total - open) / total) * 100) : 0}%</p>
                ...
                {/* Top Escalations */}
                <div className="rounded-xl bg-white p-5 shadow-sm" style={{ border: "1px solid var(--plum-border)" }}>
                  <h3 className="mb-4 text-sm font-semibold" style={{ color: "var(--plum-text)" }}>Top Escalations - Daily Shortlist</h3>
                  <div className="space-y-3">
                    ...
                    {topEscalations.map((e) => (
                      <div key={e.id} className={`flex items-center gap-4 rounded-lg border p-4 ${e.isBreachingSLA ? "border-[#E91E63]/30 bg-[#FCE4EC]/30" : ""}`} style={!e.isBreachingSLA ? { borderColor: "var(--plum-border)" } : undefined}>
                        <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold text-white" style={{ backgroundColor: e.priority === "Critical" ? "#E91E63" : "#F57C00" }}>
                          {e.priorityScore}
                        </div>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {e.isBreachingSLA && (
                              <span className="flex-shrink-0 rounded px-1.5 py-0.5 text-[10px] font-semibold text-white" style={{ backgroundColor: "#E91E63" }}>SLA BREACHED</span>
                            )}
                          </div>
                          <p className="mt-0.5 text-xs text-gray-500">{e.accountName} &middot; {e.category} &middot; {e.source}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-xs font-medium" style={{ color: "var(--plum-text)" }}>{e.owner}</p>
            ...
            {tab === "analytics" && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div className="rounded-xl bg-white p-5 shadow-sm" style={{ border: "1px solid var(--plum-border)" }}>
                    <h3 className="mb-3 text-sm font-semibold" style={{ color: "var(--plum-text)" }}>Account Size Distribution</h3>
                      ...
                      {(["Enterprise", "Mid-Market", "SME", "Startup"] as const).map((size) => {
                        ...
                        return (
                          <div key={size}>
                            ...
                            <div className="mt-1 h-2 rounded-full" style={{ backgroundColor: "var(--plum-light)" }}>
                              <div className="h-2 rounded-full" style={{ width: `${pct}%`, backgroundColor: "#5E35B1" }}></div>
                            </div>
                          </div>
                        );
                      })}
